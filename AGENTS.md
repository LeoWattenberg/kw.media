...
- Binary test fixtures live in `tests/fixtures/` and are real, decodable media generated by `npm run fixtures:test` (`scripts/generate-test-fixtures.mjs`). Prefer them over the heavy local-only files in the gitignored `reference/` directory so tests do not skip in CI.
- A test that only asserts a status label changed does not prove a tool works. When a tool produces a file, fetch the download link's blob URL inside the page and assert the real bytes: container magic numbers, decoded dimensions, sample rate, or parsed text.
