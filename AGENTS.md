...
- Run the complete suite with `npm run test:browser`. The npm pretest hook builds the site first, and `tests/browser/global-setup.mjs` starts the preview server at `http://127.0.0.1:4322` for the test run.
...
- The preview server binds only to loopback. If port 4322 is occupied, set another one for the command, for example `PLAYWRIGHT_PORT=4323 npm run test:browser`; the config uses the same value for both the server and test base URL.
- `astro preview` detaches into a background daemon when it detects that an agent is driving it, and stays in the foreground otherwise, so Playwright's own `webServer` option cannot own it: under an agent the foreground command exits at once, which Playwright reports as `Process from config.webServer exited early`, and any surviving daemon makes the next run fail with `is already used`. `tests/browser/global-setup.mjs` therefore stops a stale daemon, starts a fresh one with an explicit `--background` so both environments behave alike, and waits for it to answer; `global-teardown.mjs` stops it again. Only one preview daemon can exist per repository, so browser runs cannot be parallelised across agents — run them one at a time. Recover a stuck one with `npx astro preview stop`.
- Tools that download a WebAssembly engine at runtime (ffmpeg.wasm from unpkg, ImageMagick WASM from jsDelivr, MediaPipe from Google storage) should call `cacheCdnAssets(page)` from `tests/browser/cdn-cache.mjs` before navigating. The first run stores the responses under `.cache/browser-cdn/` and later runs replay them, which is what makes real end-to-end conversion tests affordable. Give those tests `test.setTimeout(180_000)`.
- Firefox, WebKit, and mobile-emulation projects are opt-in. Install the required browser engines, then run with `PLAYWRIGHT_CROSS_BROWSER=1`; Chromium is the only required/default project.
- Failed runs write diagnostics under `test-results/`; inspect the error output and trace artifacts there, but do not commit or manually edit that generated directory. Prefer headless runs in agent environments rather than `--ui` or `--headed`.
